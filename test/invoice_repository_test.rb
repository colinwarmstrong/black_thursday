...
class InvoiceRepositoryTest < Minitest::Test
  def setup
    sales_data = { invoices: './data/invoices.csv' }
  ...
  def test_find_by_id_returns_an_invoice_associated_to_the_given_id
    invoice_id = 3452
    expected = @invoices.find_by_id(invoice_id)
  ...
  def test_find_all_by_customer_id_returns_all_invoices_associated_with_customer
    customer_id = 300
    expected = @invoices.find_all_by_customer_id(customer_id)
  ...
  def test_find_all_by_merchant_id_returns_all_invoices_associated_with_id
    ...
    expected = @invoices.find_all_by_merchant_id(merchant_id)
  ...
  def test_find_all_by_status_returns_all_invoices_associated_with_status
    status = :shipped
    expected = @invoices.find_all_by_status(status)
  ...
  def test_create_creates_a_new_invoice_instance
    ...
    expected = @invoices.find_by_id(4986)
  ...
  def test_update_updates_an_invoice
    ...
    @invoices.create(attributes)
    original_time = @invoices.find_by_id(4986).updated_at
    attributes = {
      status: :success
    }
    @invoices.update(4986, attributes)
    expected = @invoices.find_by_id(4986)
  ...
  def test_update_cannot_update_id_customer_id_merchant_id_or_created_at
    ...
    @invoices.create(attributes)
    @invoices.update(4986, updated_attributes)
    expected = @invoices.find_by_id(5000)
    assert_nil expected
    expected = @invoices.find_by_id(4986)
  ...
  def test_delete_deletes_the_specified_invoice
    ...
    @invoices.create(attributes)
    assert @invoices.find_by_id(4986)
    @invoices.delete(4986)
    refute @invoices.find_by_id(4986)
  end

  def test_delete_on_unknown_invoice_does_nothing
    expected = @invoices.delete(5000)
    assert_nil expected
  end
end
