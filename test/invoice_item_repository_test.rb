...
class InvoiceItemRepositoryTest < Minitest::Test
  def setup
    sales_data = { invoice_items: './data/invoice_items.csv' }
  ...
  def test_create_creates_a_new_invoice_item_instance
    attributes = {
      ...
      :invoice_id => 8,
      :quantity => 1,
      :unit_price => Kernel.BigDecimal(10.99, 4),
      :created_at => Time.now,
      :updated_at => Time.now
    }
  ...
  def test_update_cannot_update_id_item_id_invoice_id_or_create_at
    ...
    @invoice_items.update(21831, updated_attributes)
  ...
  def test_update_on_unkown_invoice_item_does_nothing
    expected = @invoice_items.update(22000, {})
    assert_nil expected
  end

  def test_delete_deletes_the_specified_invoice
    assert @invoice_items.find_by_id(1)
    @invoice_items.delete(1)
    refute @invoice_items.find_by_id(1)
  end

  def test_delete_on_unknown_invoice_does_nothing
    expected = @invoice_items.delete(22000)
    assert_nil expected
  end
end
